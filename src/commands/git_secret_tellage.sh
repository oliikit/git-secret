...
function tellage {
  ...
  # Ask name and key to be added
  local age_name
  echo "What is the name of the receipent?"
  read age_name
  # Format for writing file
  age_name="#$age_name"

  local age_key
  echo "What is the receipient's key?"
  read age_key

  if [[ ! -f "$_SECRETS_DIR_KEYS_AGE_FILE" ]]; then
    touch "$_SECRETS_DIR_KEYS_AGE_FILE"
  fi

  echo -e "$age_name\n$age_key\n" >> "$_SECRETS_DIR_KEYS_AGE_FILE"
